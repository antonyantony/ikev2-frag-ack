...
response, does not advance the IKEv2 Message ID state.
...
* Fragment Acknowledgment Notify Message Status Type payload

Two Notify Message Status Types are defined: FRAGMENT_ACK_REQ, which
acknowledges fragments of an IKE request, and FRAGMENT_ACK_RES, which
acknowledges fragments of an IKE response.

The Fragment Acknowledgment notifiers (FRAGMENT_ACK_REQ and
...
format of the fragment header is specified in [[RFC7383]].

Using two distinct notifiers makes the direction of acknowledgment
explicit, avoiding ambiguity in cases of simultaneous exchanges or
delayed delivery where multiple messages share the same Message ID.
...
The payload enumerates the set of missing fragments for a single IKE
...
in [[RFC7383]], Section 2.5.  For example: 4,4; 7,10; and so on.

FRAGMENT_ACK_RES FRAGMENT_ACK_REQ or FRAGMENT_ACK_RES depends on
whether a peer is acknowledging an IKEv2 request or a response.
...
When the initiator sends a FRAGMENT_ACK_RES notification in response
...
Receipt of a FRAGMENT_ACK_REQ or FRAGMENT_ACK_RES notification
...
FRAGMENT_ACK_SUPPORTED.

* Operational Considerations


The FRAGMENT_ACK notification message SHOULD NOT be large enough to
...
When the sender receives multiple FRAGMENT_ACK messages, the sender
treats the union of all ranges acknowledged so far as the
authoritative view.

** When to Send a Fragment Acknowledgment

Since the IKEv2 retransmission model is sender-driven, the responder
...
used as IV this would lead to reuse of IV. Which MUST be avoided.

** Update to RFC7383
[[RFC7383]] specifies that when an IKE message is retransmitted, all
...
updates [[RFC7383]] by modifying the retransmission behavior specified
...
than retransmitting all fragments including fragment number 1.

Section 4 of [[RFC7383]] should be updated to allow fragment
acknowledgment.


* Editors Notes Open Issues

The following issues are retained for discussion during the evolution
of this document and are expected to be resolved or removed if the
document becomes a Working Group item.

- PMTU discovery mentioned in [[RFC7383]] should work as it is.
- This I.D. opens up possibility to send fragments of different
  sizes for PMTU detection.
- When fragments exceed the PMTU, they may not be acknowledged,
...
- Why not use IKEv2 INFORMATIONAL? That may adhere more to IKEv2.
  ...
  IKE state. This would lead to complex unpredictable retransmissions.
- Why not make new IKEv2 exchange without a response? Instead of
  ...
  protocol change. Probably too complicated?
- Should every acknowledgment sent by an IKE peer MUST include the
  ...
  packet FRAGMENT_ACK message would be partial.
