...
When the initiator sends a FRAGMENT_ACK notification in response to a
fragmented response, it MUST set the IKE header Response (R) bit.
This results in a message that is technically a
“response to a response” and reuses the same Message ID, which is
unusual in IKEv2.  However, this message is informational in nature,
does not advance the Message ID state, and does not require a
response. [NOTE: Other solutions/ideas?]
...
- Rename ACK to NACK? -ve acknowledgment?
