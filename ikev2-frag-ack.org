...
The Fragment Acknowledgment Notifier, FRAGMENT_ACK_REQ and
FRAGMENT_ACK_RES, are primarily useful during IKE_AUTH,
IKE_INTERMEDIATE, and CREATE_CHILD_SA exchanges. During IKE_AUTH and
IKE_INTERMEDIATE, peer authentication may still be incomplete.
...
eliminating the ambiguity simultaneous exchanges or delayed arrival with
...
FRFRAGMENT_ACK_RESAGMENT_ACK_REQ or FRAGMENT_ACK_RES depends on whether a peer
is acknowledging an IKEv2 request or a response.
...
This message is informational in nature, does not advance the Message ID state,
...
It is only sent after both ends have sent FRAGMENT_ACK_SUPPORTED.
